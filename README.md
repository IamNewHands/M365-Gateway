...
- 可选的会话级多账号分摊（默认关闭）：通过 `POST /api/admin/settings` 传 `{"accountSpread": true}` 启用后，新会话以持久化游标在全部健康账号间轮转绑定，每条会话粘住自己的账号形成并行泳道；凭据读取/刷新围栏、上游门控准入和账号级健康记录对分摊泳道同步放宽，但全局限流串行、每次 1 秒间隔、故障回退收敛到全局活动路由等不变量保持不变。关闭开关后新会话立即回到单活路由；该开关改变上述单活休眠保证，启用前请自行评估多账号并发的风控暴露。
- 可选的云端会话自动清理（默认关闭）：支持在管理后台设置或通过 `POST /api/admin/settings` 配置 `cloudCleanup`（包含启用开关、闲置淘汰时间 `maxAgeHours` 和每账号保留数 `keepN`），由 Cloudflare Cron 定时任务（每 2 小时扫描）或后台“立即清理”按钮自动调用微软管理面 API 清理云端堆积的闲置会话窗口。
